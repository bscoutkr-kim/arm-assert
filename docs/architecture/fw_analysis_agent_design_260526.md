...
정교한 의사결정을 위해 `Technical_Lead`(또는 `FA_Manager`)라는 조율 노드를 반드시 배치해야 합니다. 본 책임자 노드는 아래 3대 핵심 책무를 지닙니다.

1. **중재 및 검증 (Mediation)**: 분석가들의 상충되는 이론을 데이터 증거 신뢰도 기반으로 평가합니다.
2. **최종 판정 (Failure Verdict)**: 논쟁을 조율하여 가장 타당성 높은 단 하나의 **근본 원인(Root Cause)**을 확정하고 토론을 종결합니다.
3. **액션 플랜 권고 (Action Plan)**: 개발진이 즉시 대응할 수 있는 구체적인 재현 시나리오와 코드 패치 가이드라인을 최종 보고서에 합성합니다.

---

## 3.2. LangGraph 상태 공유 및 리듀서 메커니즘 (LangGraph State Sharing & Reducer Mechanism)

다른 작업 환경이나 타 PC로 프로젝트를 이관했을 때, 개발진이 에이전트 간의 데이터 전달 원리를 명확하게 이해하고 활용할 수 있도록 공유 상태의 내부 매커니즘을 상세히 정의합니다.

### 🔄 인메모리 Single Source of Truth (SSOT)
- 에이전트 노드들은 직접적으로 소켓이나 파일을 통해 통신하지 않습니다.
- 오직 전역 인메모리 딕셔너리 구조인 **`FailureState`**라는 단 하나의 객체를 공통 매개로 소통합니다. 
- 노드가 반환한 변경 사항(Delta)을 LangGraph 엔진이 취합하여 상태를 업데이트하고, 갱신된 상태를 다음 분기 노드의 파라미터로 주입하는 불변(Immutable) 상태 전이 방식을 따릅니다.

### ➕ 리듀서(Reducer)를 통한 정보의 누적 vs 대체
`FailureState` 내부의 변수들은 성격에 따라 다르게 업데이트됩니다.

1. **단순 대체 (Overwrite - 일반 필드)**
   - `next_analysis_area`나 `rounds_left` 같은 제어용 변수는 새로운 노드가 반환한 최신 값으로 계속 **덮어씌워져 대체**됩니다.
2. **리듀서 누적 (Append/Reduce - 히스토리 필드)**
   - 토론 발언 기록(`analysis_history`)은 이전 기록들이 유실되면 안 되므로, 기존 리스트 뒤에 새 발언을 **자동으로 병합(Append)**해 주는 리듀서(Reducer) 함수가 백그라운드에서 동작합니다.
   ```python
   def reduce_history(left: list, right: list) -> list:
       # 기존 토론 히스토리(left)에 새로운 발언(right)을 덧붙임
       return left + right
   ```

### 🧠 LLM 프롬프트 조립과의 연동
각 에이전트 노드는 공유 상태(`FailureState`)에서 누적된 히스토리를 끄집어내어 프롬프트 템플릿의 문맥으로 주입합니다. 이를 통해 LLM은 **"상대방이 제기한 이전 가설과 분석 팩트"**를 정확히 인지한 채로 상호 유기적인 논박과 토론을 이어나갈 수 있습니다.

### 💾 체크포인터(Checkpointer)를 통한 장애 강건성
- 그래프 실행 시 `SqliteSaver`와 같은 영속성 체크포인터를 결합하면, 각 노드 실행 직후 `FailureState` 스냅샷 전체가 물리 DB에 실시간 직렬화되어 백업됩니다.
- 분석 도중 시스템 다운이나 크래시가 터져도, 고유한 `thread_id`만 입력하면 DB에서 마지막 직전 상태를 복원하여 **토론이 끊긴 시점부터 완벽히 이어갈 수 있는 장애 복구성**을 확보합니다.
