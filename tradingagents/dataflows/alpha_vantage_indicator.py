...
def get_indicator(
    ...
    try:
        ...
        if isinstance(data, dict):
            import json
            err_msg = data.get("Error Message") or data.get("Note") or json.dumps(data)
            return f"Error retrieving {indicator} data: {err_msg}"
        
        if not isinstance(data, str):
            return f"Error: Invalid data type returned for {indicator}"
